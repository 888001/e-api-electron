...
<!-- params参数 -->
    <div class="HeaderItem">
        <div class="HeaderItemList" v-for="(item,index) in apiItem.apiQequestParams" :key="index">
            <!-- key -->
...
export default {
    ...
    methods: {
        deleteItem(index){
            this.$store.commit('ApiList/deleteOptionItem',{name: "apiQequestParams", index:index})
        },
        itemKeyBlur(){
            if(this.apiItem.apiQequestParams[this.apiItem.apiQequestParams.length - 1].itemKey && this.apiItem.apiQequestParams[this.apiItem.apiQequestParams.length - 1].itemKey.length > 0){
                    this.$store.commit('ApiList/setAPiOption',{name: "apiQequestParams", value: {...this.option}})
                }
        },
        apiKeyClick(val,index,itemKey){
            this.$store.commit('ApiList/setAPiOptionItem',{name: "apiQequestParams", value: val, index:index,'key':itemKey})
        }
    },
    computed: {
        apiItem(){
    ...
    watch: {
        'apiItem.apiQequestParams': {
            handler(news){
                if(!news || news.length == 0){
                    this.$store.commit('ApiList/setAPiName',{name: "apiQequestParams", value: [{...this.option}]})
                }
            },
            immediate: true,
            deep: true
